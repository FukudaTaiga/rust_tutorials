...
fn demo_lifetime() {
  ...
  fn longest_str<'a>(x: &'a str, y: &'a str) -> &'a str { //indicates result lifetime is the shorter one of x's or y's
    if x.len() > y.len() {
      x
    }
    else {
  ...
  struct _Life<'a> { //means structure's instance lifetime is shorter than its field
    part: &'a str,
  }
  //for method
  impl<'a> _Life<'a> {
...
fn summary() {
  ...
  println!("{}", summary_function("hello", Summary::new("sum", 45, 5.0)));
}