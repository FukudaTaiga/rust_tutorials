...
fn main() {
    // rust adopt value substitution on prelude(primitive) - deep copy (= shallow copy), clone
    //           reference substitution on the others - shallow copy, move
    // check! - Copy trait
    let mut n1 = 5;
    let n2 = n1;
    n1 = 8;
...
fn take_ref(sptr: &String) {
    // <- should be &mut String if there is a need to mutate
...
// reference doesnt have `owner`!
// reference rule:
//   1. you can take `1 mutable ref` or `some immutable refs`
//   2. ref must be always valid (null safety)
